...
$query=mysqli_query($conn,"update post set Is_Active=1 where id='$postid'");
if($query)
{
...
<?php
$query=mysqli_query($conn,"select post.id as postid,post.PostTitle as title,category.CategoryName as category,subcategory.Subcategory as subcategory from post left join category on category.id=post.CategoryId left join subcategory on subcategory.SubCategoryId=post.SubCategoryId where post.Is_Active=0");
